...
pub(crate) fn compile_lz4(
    ...
    for (index, head) in topology.heads.iter().enumerate() {
        ...
        if let Some(fragment) = topology
            .fragment_tail
            .filter(|fragment| fragment.head_lcn == head.lcn && fragment.overlay.is_some())
        {
...
fn encode_shared_fragment_overlay(
    ...
    for extent in overlay.extents.iter().flatten().take(overlay.extent_count) {
        ...
        let (block, encoded_len) =
            encode_extent(fragment.head_lcn, &decoded, Lz4Placement::LegacyStart)?;
...
fn compile_blocks(
    ...
    let extra_fragment_blocks = fragment_overlay
        .as_ref()
        ...
        .enumerate()
    {
        ...
        let materialized_pcluster =
            materialize_direct_block(&mut view, topology, *origin_head, &encoded, encoded_len)?;
...
impl Image {
    ...
    fn recover_full_fragment_tail(
        ...
        let aligned = fragment_offset % u64::from(BLOCK_SIZE) == 0
    ...
    fn recover_stage43_shared_partial_overlay(
        &mut self,
        packed_nid: u64,
        fragment_offset: u64,
        fragment_size: u64,
        ...
        let packed_size =
            usize::try_from(packed.size).map_err(|_| CoreError::ArithmeticOverflow)?;
