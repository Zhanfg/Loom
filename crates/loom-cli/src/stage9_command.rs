...
use loom_erofs::compile_same_size_replacement;
use std::error::Error;
use std::fs;
use std::path::Path;

pub(crate) fn command(args: &mut impl Iterator<Item = String>) -> Result<(), Box<dyn Error>> {
    let origin_image = required(args, "origin EROFS image")?;
    ...
    println!(
        "erofs replacement compiled: nid={} block_size={} start_block={} shadow_blocks={} shadow_bytes={}",
        compiled.nid,
        compiled.block_size,
        compiled.start_block,
