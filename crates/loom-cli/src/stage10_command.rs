...
use loom_erofs::compile_full_pcluster_swap;
use std::error::Error;
use std::fs;
use std::path::Path;
...
pub(crate) fn command(args: &mut impl Iterator<Item = String>) -> Result<(), Box<dyn Error>> {
    ...
    let replacement_image = required(args, "encoded replacement EROFS image")?;
    ...
    let compiled = compile_full_pcluster_swap(
    ...
    println!(
        "erofs pcluster swap compiled: origin_nid={} origin_pcluster={} replacement_pcluster={} block_size={} shadow_blocks={} shadow_bytes={}",
        compiled.origin_nid,
