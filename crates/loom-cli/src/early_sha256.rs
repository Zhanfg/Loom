...
impl Sha256 {
    ...
    fn finalize(mut self) -> [u8; 32] {
        let bit_len = self.bytes.checked_mul(8).expect("SHA-256 bit length overflow");
...
mod tests {
    ...
    fn known_vectors() {
        ...
        assert_eq!(
            sha256_bytes_hex(b"The quick brown fox jumps over the lazy dog"),
    ...
    fn streaming_matches_single_update() {
        ...
        let expected = sha256_bytes_hex(&payload);
        let mut hasher = Sha256::new();
        for chunk in payload.chunks(997) {
            hasher.update(chunk);
        }
        assert_eq!(hex(&hasher.finalize()), expected);
    }
}
