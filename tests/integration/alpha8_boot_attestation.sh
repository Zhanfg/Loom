...
gcc -std=c11 -O2 -Wall -Wextra -Werror tools/loom-early-attest-host.c -o "$ATTEST_BIN"
...
# A candidate cannot be attested before recovery state authorizes the exact attempted generation.
...
# Next boot creates a new epoch and must erase the previous active record before decision.
...
# New boot epoch clears B's stale active marker. Recovery then quarantines B and selects A.
...
# A last-good fallback must not be able to confirm A as if desired B had succeeded.
...
# Even if an old active record is recreated after a new boot epoch, boot-id binding rejects it.
...
# Malformed or truncated boot ids are not accepted at begin/verify boundaries.
