...
grep -q 'fn recover_big_extents' "$CORE"
grep -q 'fn reconstruct_big_head_pcluster' "$CORE"
grep -q 'fn validate_big_extent' "$CORE"
grep -q 'D0_CBLKCNT' "$CORE"

grep -q 'shared_core::compile_big_oracle' "$ADAPTER"
grep -q 'shared_core::compile_big_lz4' "$ADAPTER"

if grep -Eq 'fn (read_superblock|read_inode|read_compact_entry|compact_regions|compact_entry_position|recover_big_extents|reconstruct_big_head_pcluster|validate_big_extent)' "$ADAPTER"; then
...
grep -q 'later_big_head_reconstruction_accumulates_prior_cblkcnt' "$CORE"
...
printf '%s\n' \
  ...
  '  vector big-topology recovery owner: compact_core' \
