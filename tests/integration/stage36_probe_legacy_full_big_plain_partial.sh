...
import random, sys
size = 98181
prefix = bytearray((b'LOOM-STAGE36-COMPRESS-' * 4000)[:65536])
rng = random.Random(0x360001)
tail = bytearray(rng.randrange(256) for _ in range(size - len(prefix)))
data = prefix + tail
...
print('STAGE36_SB incompat=',hex(struct.unpack_from('<I',raw,sb+0x50)[0]))
...
for nid in range(int(inos)+16):
    ...
    print(f'STAGE36_TARGET nid={nid} data_word={blocks} header={h.hex()}')
    for lcn in range(24):
        advise,co,word=struct.unpack_from('<HHI',raw,start+lcn*8)
        print(f'STAGE36_IDX lcn={lcn} advise=0x{advise:04x} type={advise&3} clusterofs={co} word=0x{word:08x} delta0=0x{word&0xffff:04x} delta1={word>>16}')
    break
