...
ORIGIN_LOOP=""; SHADOW_LOOP=""; MAPPER="loom-stage36-${RANDOM}-${RANDOM}"
cleanup() {
  set +e
...
trap cleanup EXIT
trap 'rc=$?; printf "Stage 36 FAIL line=%s status=%s command=%s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2; exit "$rc"' ERR
mkdir -p "$ROOT" "$MNT"
...
def build(seed, tag):
    ...
    return prefix+tail
open(sys.argv[1],'wb').write(build(0x360001,b'LOOM-STAGE36-ORIGIN'))
open(sys.argv[2],'wb').write(build(0x360002,b'LOOM-STAGE36-REPLACEMENT'))
PY
...
print(f'Stage 36 raw full-big partial PLAIN topology PASS heads={heads} data_word={blocks}')
PY
...
assert v[9]==3973,v
print(f'Stage 36 encoded full-big partial raw tail PASS encoded={v}')
PY
...
printf '%s\n' \
  'Stage 36 legacy full-index big-pcluster partial PLAIN raw tail PASS' \
  '  logical bytes: 98181' \
  '  HEAD1 lclusters: [0, 8]' \
  '  PLAIN data lclusters: [16, 17, 18, 19, 20, 21, 22, 23]' \
  '  final PLAIN logical bytes: 3973; physical bytes: 4096' \
  '  physical footprints: 10 x 1 block' \
