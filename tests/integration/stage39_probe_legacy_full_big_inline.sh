...
data=bytearray(periodic(0x390101))
data.extend(periodic(0x390102))
data.extend((b'LOOM-STAGE39-INLINE-TAIL-'*2000)[:EXTENT])
assert len(data)==98304
open(sys.argv[1],'wb').write(data)
PY
for i in $(seq -w 0 499); do : > "$WORK/root/z_dummy_${i}_for_directory_growth"; done

mkfs.erofs -b 4096 -C 16384 -zlz4 -E legacy-compress,ztailpacking -T 0 \
...
compat=struct.unpack_from('<I',raw,sb+0x08)[0]
incompat=struct.unpack_from('<I',raw,sb+0x50)[0]
print('STAGE39_SB compat=',hex(compat),'incompat=',hex(incompat))
...
for nid in range(int(inos)+16):
    ...
    print(f'STAGE39_TARGET nid={nid} inode_off={off} isize={isize} xsize={xsize} data_word={blocks} header_off={header} header={h.hex()}')
    for lcn in range(24):
        advise,co,word=struct.unpack_from('<HHI',raw,start+lcn*8)
        print(f'STAGE39_IDX lcn={lcn} advise=0x{advise:04x} type={advise&3} clusterofs={co} word=0x{word:08x} delta0=0x{word&0xffff:04x} delta1={word>>16}')
    end=start+24*8
    print('STAGE39_AFTER_INDEX offset=',end,'first256=',raw[end:end+256].hex())
    break
