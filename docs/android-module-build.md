...
4. Optionally set `source_ref` to a branch, tag, or commit SHA. Leaving it empty builds the workflow run's source commit.
5. Download the `Loom-Android-*` artifact after the job completes.

The artifact contains the module ZIP and its SHA-256 file.

## Current packaging-alpha boundary
...
The packaging format and CI workflow are designed to remain stable when runtime activation is added; only the module scripts need to gain the activation logic.
